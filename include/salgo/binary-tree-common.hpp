...
namespace internal {
	...
	// vertex value
	//
	template<bool, class T> struct Vert_Add_value         {};
	template<      class T> struct Vert_Add_value<true,T> {

		T value = T();

		template<class TT>
		inline auto& operator=(TT&& tt) {
			value = std::forward<TT>(tt);
			return *this;
		}

		Vert_Add_value() = default;

		template<class... Args>
		Vert_Add_value(Args&&... args) : value(std::forward<Args>(args)... ) {}
	};
}
